...
@WebServlet("/SortByLowestPriceServlet")
public class SortByLowestPriceServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Knife> knifesSortedByLowestPrice = new ArrayList<>();
		try {
			knifesSortedByLowestPrice = knifeDAO.getAllKnifesSortByLowestPrice();
		...
		session.setAttribute("allKnifes", knifesSortedByLowestPrice);
		//request.getRequestDispatcher("UserViewAllKnifes.jsp").forward(request, response);
		
		JsonArray array = new JsonArray();
		for (Knife knife : knifesSortedByLowestPrice) {
